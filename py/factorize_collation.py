...
"""
def main():
    parser = argparse.ArgumentParser(description="Reads a collation into a matrix and factorizes the matrix into basis (i.e., group reading profile) and mixture (i.e., witness grouping) factors.")
    ...
    parser.add_argument("output", type=str, help="Filename for the Excel (.xlsx) or JSON (.json) output containing summary metrics and NMF factors for the primary and fragmentary collation data.")
    parser.add_argument("rank", type=int, help="Desired rank of the NMF factors. Good candidate values can be determined with the estimate_rank.py script.")
    ...
    if input_addr.endswith(".xml"):
        ...
        try:
            ...
            exit(1)
    # Then initialize a collation_factorizer instance and perform NMF:
    cf = collation_factorizer(cp, verbose=verbose)
    cf.factorize_collation(rank)
    # Then write the results to the appropriate output:
    if output_addr.endswith(".xlsx"):
        cf.to_excel(output_addr)
    elif output_addr.endswith(".json"):
        json_output = cf.to_json()
